...
* The organization has some existing directory or mapping of individuals to their team or business unit (an organization chart, HR system, or internal directory) that can be used to determine a contributor's home team.
...
Replace informal, ask-around discovery of cross-team collaboration with a programmatic scan of contribution data against the organization's team structure, then follow up manually on what the scan surfaces.

**1. Establish a contributor-to-team mapping.** Use whatever directory the organization already maintains (an organization chart, HR system, or internal people directory) to determine which team or business unit each individual contributor belongs to. This mapping is the foundation the rest of the detection depends on, and it needs to be kept reasonably current as teams reorganize.
